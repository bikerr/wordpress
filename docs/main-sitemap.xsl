			...
			<head>
				<xsl:choose>
					<xsl:when test="kml:kml">
						<title>Locations Sitemap - iPassportPhoto Blog</title>
					</xsl:when>
					<xsl:otherwise>
						<title>XML Sitemap - iPassportPhoto Blog</title>
					</xsl:otherwise>
				</xsl:choose>
				...
				<xsl:choose>
					<xsl:when test="kml:kml">
						...
						<div id="content">
							...
							<p class="expl">
								<a href="https://blog.ipassportphoto.com/sitemap_index.xml">&#8592; Sitemap Index</a>							</p>
